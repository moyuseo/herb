...
<template>
  <div class="min-h-screen bg-[#FAF8F5]" v-if="herb">
    <div class="max-w-4xl mx-auto px-4 py-6">
      <button @click="router.back()" class="flex items-center gap-1 text-sm mb-4 text-[#1B5E20] hover:underline">
        <ArrowLeft :size="18" /> 返回
      </button>

      <div class="bg-gradient-to-br from-[#1B5E20] to-[#2E7D32] rounded-xl p-6 mb-6 text-white">
      ...
      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100 mb-4">
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div v-for="item in infoItems" :key="item.label" class="flex flex-col">
            ...
            <span class="text-sm text-[#1B5E20] font-medium mt-0.5">{{ item.value }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100 mb-4">
        <div class="flex items-center gap-2 mb-2">
          <Leaf :size="16" class="text-[#1B5E20]" />
          <h3 class="font-bold text-[#1B5E20] text-sm">功效</h3>
        </div>
        <p class="text-sm text-gray-600 leading-relaxed">{{ herb.efficacy }}</p>
      </div>

      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100 mb-4">
        <h3 class="font-bold text-[#1B5E20] text-sm mb-2">用法用量</h3>
        <p class="text-sm text-gray-600 leading-relaxed">{{ herb.usage }}</p>
      </div>

      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100 mb-4">
        <h3 class="font-bold text-[#1B5E20] text-sm mb-2">禁忌</h3>
        <p class="text-sm text-red-500 leading-relaxed">{{ herb.contraindication }}</p>
      </div>

      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
        <h3 class="font-bold text-[#1B5E20] text-sm mb-2">详细描述</h3>
